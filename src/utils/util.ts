 ...
 */
export function formatSeason(dateStr) {
 ...
 */
export function isArrayEqualLoose(arr1, arr2) {
    // 第一步：长度不一致直接返回false
 ...
 */
export function findElementIndex(element, arr) {
    // 前置校验：确保传入的是合法数组
 ...
 * @returns {number} 元素索引（不存在返回 -1）
 */
export function findElementIndexStrict(element, arr, comparator) {
    if (!Array.isArray(arr)) {
        console.warn('第二个参数必须是数组');
        return -1;
    }
    // 自定义对比函数优先级更高，否则用全等（===）对比
    const compare = comparator || ((item) => item == element);
