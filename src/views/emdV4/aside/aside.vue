    ...
    <div class="aside">
        ...
        <div class="emd-aside-bottom">
            ...
            <el-popover placement="right" :width="200" trigger="hover">
                ...
                <el-menu :default-active="emdStore.getCurrentMode">
    ...
    switch (emdStore.currentMode) {
        case "阅读":
            if (node.level == 2) {
                ...
                router.push({ name: 'emdV4LabRead', query: { labId: data.id } })
            }
            break;
        case "编辑":
            switch (node.level) {
                case 4:
                    ...
                    router.push({ name: 'emdV4SectionEdit', query: { section: data.id } })
                    break;
                default:
                    router.push({ name: 'emdv4' })
                    break;
            }
            return;
        default:
    ...
    switch (newVal) {
        case "阅读":
            if (currentNode.value.level == 2) {
                emdStore.setRouterKey('labProc-read-' + currentNode.value.data.treeId)
                router.push({ name: 'emdV4LabRead', query: { labId: currentNode.value.data.id } })
            }
            break;
        case "编辑":
            if (currentNode.value.level == 2) {
            ...
            if (currentNode.value.level == 4) {
                ...
                router.push({ name: 'emdV4SectionEdit', query: { section: currentNode.value.data.id } })
            }
            break;
        case "答案":
            if (currentNode.value.level == 2) {
                emdStore.setRouterKey('labProc-answer-' + currentNode.value.data.treeId)
                router.push({ name: 'emdV4SectionAnswer', query: { labId: currentNode.value.data.id } }) // 答案设定
            }
            break;
        case "参考":
            if (currentNode.value.level == 2) {
                emdStore.setRouterKey('labProc-reference-' + currentNode.value.data.treeId)
                router.push({ name: 'emdV4LabReference', query: { labId: currentNode.value.data.id } }) // 参考资料设定
            }
            break;
        default:
            return;
