<template>
    <div v-if="payload" ref="choiceRef" :id="'block' + blockDetail.id"
        ...
        <div v-if="payload.isMultiple">
            <el-checkbox-group class="select-item scroll-mt-[80px]" :id="payload.cell.id" disabled
                v-model="payload.cell.stuValue.array">
            ...
            <el-checkbox-group class="select-item scroll-mt-[80px] mt-4 bg-green-100" :id="payload.cell.id" disabled
                v-model="payload.cell.presetValue.array">
                <el-checkbox class=" rounded hover:bg-gray-100" v-for="(item, i) in payload.options"
                    :key="'check-' + blockDetail.id + '-' + i" :label="item" style="height: auto;">
                    <TextPreview :id="'option-' + blockDetail.id + '-check-' + i" :content="item"
                        class="scroll-mt-[80px]" />
        ...
        <div v-else>
            <el-radio-group class="select-item scroll-mt-[80px]" :id="payload.cell.id" disabled
                v-model="payload.cell.stuValue.array[0]">
            ...
            <el-radio-group class="select-item scroll-mt-[80px] mt-4 bg-green-100" :id="payload.cell.id" disabled
                v-model="payload.cell.presetValue.array[0]">
                <el-radio class="rounded hover:bg-gray-100" v-for="(item, i) in payload.options" :label="item"
                    style="height: auto;">
                    <TextPreview :id="'option-' + blockDetail.id + '-radio-' + i" :content="item"
                        class="scroll-mt-[80px]" />
...
import { generateShortUUID, type PAYLOAD, type blockVo, upCell } from '../../ts/EMDLab';
import TextPreview from '@/views/gradeTest/emdv2/textPreview/textPreview.vue';
