<template>
  <el-dialog v-model="dialog.visible" :title="dialog.title" :before-close="close">
    <div class="mb-4">
    ...
    <el-table :data="list">
      ...
      <el-table-column label="操作" width="180">
        <template #default="{ row }">
          <el-button :disabled="!row.finished" @click="hanldeDownload(row.resource.filename)" type="primary" plain icon="Download">下载</el-button>
        </template>
      </el-table-column>
    </el-table>
  </el-dialog>
...
import { generateStudentGradeReport, reGenerate, showProgress } from '@/apis/embV4/report'
...
const initGenReport = () => {
  ...
  generateStudentGradeReport(projectId.value).then(res => {
    if (res.state == 200) {
...
const handleReGenerate = () => {
  ...
  reGenerate(projectId.value).then(res => {
    if (res.state == 200) {
      if (res.data.errorMsg) {
        ElMessage.error(res.data.errorMsg)
        return
