...
import { GetTask } from '@/apis/task/getTaskById.js';
import { GetStudentTaskChat } from '@/apis/aiAssistant/studentTaskChat';
...
const labInit = () => {
    //todo 获取学生的task的chatId
    GetStudentTaskChat(studentId.value, taskId.value).then(res => {
        if (res.state == 200) {
...
onMounted(() => {
    ...
    setTimeout(() => {
        ...
        labStore.setStudentId(studentId.value)
